...
- **Headers go only to the site's hosts.** A site's `fetch.headers` are sent only to the hosts named by its discovery URLs, plus any listed in `fetch.header_hosts`, matched exactly (`example.com` is not `www.example.com`). They are removed from any redirect hop or fresh request to another host, including that host's `robots.txt`, and a warning names each such host once per site. The fetcher's own headers (User-Agent and the conditional-GET validators) are always sent, and setting them in `fetch.headers` is a config error; use `user_agent` in `feed-me.yaml` to change the User-Agent.
- **IPv6 that embeds IPv4.** IPv4-compatible (`::/96`), NAT64 (`64:ff9b::/96`, `64:ff9b:1::/48`), Teredo (`2001::/32`) and 6to4 (`2002::/16`) addresses are refused too, since they can reach private IPv4 addresses.
...
fetch:
  ...
  header_hosts: []          # more hosts that get them, e.g. [www.example.com]
  respect_robots: true
