...
- **Headers go only to the site's hosts.** A site's `fetch.headers` are sent only to the hosts named by its discovery URLs, plus any listed in `fetch.header_hosts`, matched exactly (`example.com` is not `www.example.com`). They are removed from any redirect hop or fresh request to another host, including that host's `robots.txt`, and a warning names each such host once. The fetcher's own headers (User-Agent and the conditional-GET validators) are always sent.
